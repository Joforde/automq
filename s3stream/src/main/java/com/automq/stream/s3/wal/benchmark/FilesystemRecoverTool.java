...
import com.automq.stream.s3.wal.impl.filesystem.FilesystemWALHeader;
 ...
 * It extends {@link FilesystemWALService} to use tools provided by {@link FilesystemWALService}
 */
public class FilesystemRecoverTool extends FilesystemWALService implements AutoCloseable {

    @SuppressWarnings("this-escape")
    public FilesystemRecoverTool(Config config) throws IOException {
        super(FilesystemWALService.recoveryBuilder(config.path));
        super.start();
    }
    ...
    public static void main(String[] args) throws IOException {
        ...
        try (FilesystemRecoverTool tool = new FilesystemRecoverTool(config)) {
            tool.run(config);
        }
    }

    private void run(Config config) throws IOException {
        FilesystemWALHeader header = super.tryReadLatestHeader();
        System.out.println(header);

        Iterable<RecoverResult> recordsSupplier = () -> recover(header, config);
        Function<ByteBuf, String> stringer = buf -> {
            StreamRecordBatch batch = StreamRecordBatchCodec.decode(buf);
            try {
                return StreamRecordPayloadFormatter.format(batch);
            } finally {
                batch.release();
            }
        };
        Function<Long, String> offsetStringer = offset -> String.format("%016x", offset);
    ...
    private Iterator<RecoverResult> recover(FilesystemWALHeader header, Config config) {
        long recoverOffset = header.getTrimOffset();
        return new RecoverIterator(recoverOffset);
    }
...
//            inner.record().release();
        ...
        public String toString() {
            ...
            return String.format("%s{offset=%s", inner.getClass().getSimpleName(), offset)
                + String.format(", record=(%d)", inner.record().readableBytes())
                + stringer.apply(inner.record())
                + '}';
        }
    }
